            ...
            <div class="col-12">
                <div class="row d-flex justify-content-center mt-5">
                    <div class="col-12 mb-4">
                        <h4 class="text-center">Sipariş Düzenle</h4>
                    </div>
                    <div class="col col-md-3 rounded-4 p-5" style="background: #1e2833">
                        <form action="{{ route('orderStatusChange', $order->id) }}" method="POST">
                            @csrf

                            <div class="mb-5">
                                <label for="siparis" class="form-label text-white">Sipariş Durumu</label>
                                <select class="form-select" name="siparis" id="siparis">
                                    <option selected>{{ $order->orders }}</option>

                                    @if ($order->orders == 'Onay Bekliyor')
                                        <option value="Kargoya Verildi">Kargoya Verildi</option>
                                    @elseif ($order->orders == 'Kargoya Verildi')
